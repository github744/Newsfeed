...
public class NewsDao {
    ...
    public boolean removeById(int id) {
        ...
        try {
            ...
            if (con != null) {
                ...
                String sql1 = "Delete from ncr where news_id=?";
                PreparedStatement smt1 = con.prepareStatement(sql1);
                smt1.setInt(1, id);

                int n1 = smt1.executeUpdate();
    ...
    public ArrayList<News> getAllNews() {
      ...
      public ArrayList<News> getNewsByKeyword(String keyword) {
        ...
        try {
            ...
            Connection con = cp.getConnection();
            if (con != null) {
                String sql = "select * from news where id in(select news_id from ncr where cat_id in(select id from news_cat where name like '%"+keyword +"%'))";
                PreparedStatement smt = con.prepareStatement(sql);
    ...
    public ArrayList<News> getNewsByLimit(String status ,int start, int stop ) {
        ArrayList<News> newsList = new ArrayList();
        try {
            ConnectionPool cp = ConnectionPool.getInstance();
            cp.initialize();
            Connection con = cp.getConnection();
            if (con != null) {
                String sql = "select * from news where status=? order by id desc limit ?, ?";
                PreparedStatement smt = con.prepareStatement(sql);
    ...
    public ArrayList<News> getNewsByCatId(int catid,String status) {
        ArrayList<News> newsList = new ArrayList();
        try {
            ConnectionPool cp = ConnectionPool.getInstance();
            cp.initialize();
            Connection con = cp.getConnection();
            if (con != null) {
                String sql = "select * from news where id in (select news_id from ncr where cat_id =?)and status=? order by id desc";
                PreparedStatement smt = con.prepareStatement(sql);
                smt.setInt(1, catid);
      ...
      public ArrayList<News> getNewsByCatIdLimit(int catid,String status,int limit) {
        ArrayList<News> newsList = new ArrayList();
        try {
            ConnectionPool cp = ConnectionPool.getInstance();
            cp.initialize();
            Connection con = cp.getConnection();
            if (con != null) {
                String sql = "select * from news where id in (select news_id from ncr where cat_id =?)and status=? order by id desc limit ?";
                PreparedStatement smt = con.prepareStatement(sql);
                smt.setInt(1, catid);
                 smt.setString(2, status);
                smt.setInt(3, limit);
